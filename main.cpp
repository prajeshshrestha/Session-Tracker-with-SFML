...
int main()
{
	...
	video_mode.width = 760.f;
	video_mode.height = 675.f;
	sf::RenderWindow window(video_mode, "SESSION APP", sf::Style::Titlebar | sf::Style::Close, settings);
	sf::Event event;

	sf::View session_list_view;
	session_list_view.reset(sf::FloatRect(0, 0, 760.f, 675.f));
	session_list_view.setViewport(sf::FloatRect(0, 0.3035, 1.f, 1.f));
	...
	while (window.isOpen())
	{
		while (window.pollEvent(event))
		{
			...
			session_app.Run_InputField_Event(window, event);
		}
